...
export default function Navbar({ variant = "home" }) {
  const showButtons = variant === "project";
  ...
  if (variant === "home") {
    return (
      <>
        <div className="absolute top-0 left-0 w-full z-50 px-4 md:px-10 pt-4">
          <div
            className="h-[78px] rounded-none md:rounded-b-[26px] px-4 md:px-10
          ...
          >
            ...
            <Image
              ...
              width={110}
              height={60}
              className="object-contain mt-6 w-[90px] md:w-[110px]"
            />
  ...
  transition-all duration-300"
            ...
            <button
              ...
              className="md:hidden flex items-center bg-[#9f773c] rounded-full px-4 py-1 shadow-lg text-white text-3xl"
            >
              {isOpen ? "✕" : "☰"}
            </button>
          </div>
        ...
        <AnimatePresence>
          {isOpen && (
            <motion.div
              ...
              exit={{ opacity: 0, y: -20 }}
              className="fixed inset-0 z-[90] md:hidden bg-[#C4934A] backdrop-blur-xl flex flex-col items-center justify-center"
            >
  ...
  return (
    <>
      <div className="w-full flex items-center justify-between lg:grid lg:grid-cols-[1fr_auto_1fr] px-3 lg:px-7 z-50 relative lg:absolute lg:top-0 lg:left-0 bg-[#F7F3EC] lg:bg-transparent">
        {/* LOGO */}
        <Image
          src="/logo.png"
          alt="Logo"
          width={110}
          height={60}
          className="object-cover w-[85px] lg:w-[100px]"
        />

        {/* MENU SECTION */}
        <div className="flex items-center bg-[#9f773c] backdrop-blur-md rounded-full px-3 py-1 shadow-lg">
          ...
          <button
            onClick={() => setIsOpen(!isOpen)}
            className="lg:hidden text-white text-3xl px-2"
          >
            {isOpen ? "✕" : "☰"}
          </button>
        </div>
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            animate={{ opacity: 1, y: 0 }}
            exit={{ opacity: 0, y: -20 }}
            className="fixed inset-0 z-[90] lg:hidden bg-[#C4934A] backdrop-blur-xl flex flex-col items-center justify-center"
          >
            <div className="flex flex-col gap-8 text-[#1E3D34] text-2xl font-semibold">
              <a href="#" onClick={() => setIsOpen(false)}>
                Home
      ...
      <EnquiryForm open={open} setOpen={setOpen} variant="popup" />
    </>
  );
}
